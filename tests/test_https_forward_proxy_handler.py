...
class TestHTTPSForwardProxyHandler:
    ...
    async def test_on_request_received_calls_forward_http_request(self):
        ...
        with patch.object(handler, 'forward_http_request', new_callable=AsyncMock) as mock_forward:
    ...
    async def test_handle_http_request(self):
        ...
        with patch.object(handler, 'read_request_body', return_value=mock_read_request_body()):            
            with patch('asyncio.open_connection', new_callable=AsyncMock) as mock_open:
                ...
                with patch.object(handler, '_read_and_forward_response', new_callable=AsyncMock):
    ...
    async def test_read_and_forward_response(self):
        ...
        with patch.object(handler, 'on_response_received', new_callable=AsyncMock) as mock_on_response:
            with patch.object(handler, '_forward_response_body', new_callable=AsyncMock):
    ...
    async def test_error_handling_connection_failed(self):
        ...
        with patch.object(handler, 'read_request_body', return_value=mock_read_request_body()):
            with patch('asyncio.open_connection', side_effect=ConnectionRefusedError):
                with patch.object(handler, 'on_response_complete', new_callable=AsyncMock) as mock_complete:
    ...
    async def test_on_response_chunk_can_modify_data(self):
        ...
        with patch.object(handler, 'write_response') as mock_write:
            with patch.object(handler, 'flush_response', new_callable=AsyncMock):
                with patch.object(handler, 'on_response_complete', new_callable=AsyncMock):
                    await handler._forward_response_body()
    ...
    async def test_response_complete_called_on_exception(self):
        ...
        with patch.object(handler, 'read_request_body', return_value=AsyncMock()) as mock_read_body:

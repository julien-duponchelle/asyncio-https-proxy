...
def test_ca_certificate_properties(tls_store):
    ...
    assert validity_period.days >= 365 * 10 - 1  # Allow for slight timing differences
...
def test_get_ca_pem(tls_store):
    ...
    assert ca_cert_from_pem.not_valid_before_utc == original_ca_cert.not_valid_before_utc
...
def test_generate_cert_creates_valid_certificate(tls_store):
    ...
    assert validity_period.days == 10
...
def test_get_ssl_context_different_domains_get_different_certs(tls_store):
    ...
    ssl_context1 = tls_store.get_ssl_context(domain1)
    ssl_context2 = tls_store.get_ssl_context(domain2)
...
def test_certificate_key_identifiers(tls_store):
    ...
    ee_aki = ee_cert.extensions.get_extension_for_class(x509.AuthorityKeyIdentifier).value
...
def test_certificate_validity_dates(tls_store):
    ...
    # CA certificate should be valid now and for ~10 years
    ...
    assert ca_validity_days >= 365 * 10 - 1
    
    # End-entity certificate should be valid now and for 10 days
...
def test_multiple_contexts_same_domain_use_same_cert(tls_store):
    ...
    assert all(isinstance(ctx, ssl.SSLContext) for ctx in [context1, context2, context3])
